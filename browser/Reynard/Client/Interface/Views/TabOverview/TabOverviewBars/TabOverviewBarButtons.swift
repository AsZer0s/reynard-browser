...
final class TabOverviewBarButtons {
    ...
    lazy var modeControl: UISegmentedControl = {
        let control = UISegmentedControl(items: ["Private", "0 个标签页"])
    ...
    func setTabCount(_ tabCount: Int) {
        modeControl.setTitle("\(tabCount)" + (tabCount == 1 ? " 个标签页" : " 个标签页"), forSegmentAt: TabOverviewCollection.Mode.regularTabs.rawValue)
